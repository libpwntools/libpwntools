...
int Socket::socket_write(std::string msg){
...
int Socket::socket_safe_read(std::string &buf,int len,int seconds){
...
int Socket::socket_read(std::string &buf,int len){
    char buffer[len];
    bzero(buffer,len);
    int status = (int)recv(sock, buffer, len-1, 0);
    if (status < 0) {
...
int Socket::socket_writeTo(std::string msg, std::string ip, std::string port){
    const char * buf = msg.c_str();
    int len = (int)strlen(buf);
    address = ip;
    this->port = port;
...
int Socket::socket_readFrom(std::string &buf, int len, std::string ip, std::string port){
    char buffer[len];
    bzero(buffer,len);
    struct addrinfo *res;
    int status;
...
std::string Socket::ipFromHostName(std::string hostname){
    ...
    if(record == NULL)
    {
        std::cerr << "Is unavailable: " << hostname << std::endl;
        exit(1);
    }
